pipeline {
    ...
    stages {
        ...
        stage('publish') {
            ...
            steps {
                script {
                    env.GITEA_USER = sh(script: "echo $GIT_URL | awk -F'/' '{ print $4 }'", returnStdout: true).trim()
                    withCredentials([string(credentialsId: 'gitea-user-${env.GITEA_USER}-full', variable: 'SECRET')]) {
                        sh "build/publish.sh"
                    }
                }
            }
